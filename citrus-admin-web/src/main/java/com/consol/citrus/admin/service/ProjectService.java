            ...
            try {
                ...
                if (!pomXml.contains("<artifactId>citrus-admin-connector</artifactId>")) {
                    ...
                    for (String pattern : patterns) {
                        ...
                        if (matcher.find()) {
                            pomXml = pomXml.substring(0, matcher.end()) + String.format("%n    <dependency>%n      <groupId>com.consol.citrus</groupId>%n      <artifactId>citrus-admin-connector</artifactId>%n      <version>1.0.2-SNAPSHOT</version>%n    </dependency>") + pomXml.substring(matcher.end());
                            break;
                        }
                    }
